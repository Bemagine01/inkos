...
连续性审计员从 33 个维度检查每一章草稿：角色记忆、物资连续性、伏笔回收、大纲偏离、叙事节奏、情感弧线等。内置 AI 痕迹检测维度，自动识别"LLM 味"表达（高频词、句式单调、过度总结）。默认长篇写作链路最多自动修订一次，仍有关键问题会保留结果并标记给人工或后续修订判断。
...
| **修订者 Reviser** | 修复审计发现的关键问题；默认写作链路最多自动修订一次，其他问题标记给人工审核 |

如果审计不通过，默认管线只做一次"修订 → 再审计"；仍未解决的问题会保留在结果和状态里，交给人工或后续命令继续处理。
